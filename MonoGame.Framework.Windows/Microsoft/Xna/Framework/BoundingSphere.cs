...
namespace Microsoft.Xna.Framework
{
  public struct BoundingSphere : IEquatable<BoundingSphere>
  {
    ...
    {
      Vector3 vector3 = Vector3.Subtract(additional.Center, original.Center);
      ...
      vector3 += (float) (((double) num2 - (double) num3) / (2.0 * (double) vector3.Length())) * vector3;
      return new BoundingSphere()
      {
        Center = original.Center + vector3,
    ...
    {
      float num = Vector3.Dot(plane.Normal, this.Center) + plane.D;
      if ((double) num > (double) this.Radius)
        return PlaneIntersectionType.Front;
      return (double) num < -(double) this.Radius ? PlaneIntersectionType.Back : PlaneIntersectionType.Intersecting;
    }
