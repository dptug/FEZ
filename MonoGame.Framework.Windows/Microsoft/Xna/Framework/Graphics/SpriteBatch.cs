...
namespace Microsoft.Xna.Framework.Graphics
{
  public class SpriteBatch : GraphicsResource
  {
    ...
    {
      this.CheckValid(texture);
      this.DrawInternal(texture, new Vector4((float) destinationRectangle.X, (float) destinationRectangle.Y, (float) destinationRectangle.Width, (float) destinationRectangle.Height), sourceRectangle, color, rotation, new Vector2(origin.X * ((float) destinationRectangle.Width / (float) texture.Width), origin.Y * ((float) destinationRectangle.Height / (float) texture.Height)), effect, depth);
    }
