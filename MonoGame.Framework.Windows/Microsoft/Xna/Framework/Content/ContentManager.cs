...
namespace Microsoft.Xna.Framework.Content
{
  public class ContentManager : IDisposable
  {
    ...
    internal string RootDirectoryFullPath
    {
      get
      {
        return Path.Combine(AppDomain.CurrentDomain.BaseDirectory, this.RootDirectory);
      }
    }
    ...
    protected virtual Stream OpenStream(string assetName)
    {
      ...
      try
      {
        stream = TitleContainer.OpenStream(Path.Combine(this.RootDirectoryFullPath, assetName) + ".xnb");
      }
    ...
    protected T ReadAsset<T>(string assetName, Action<IDisposable> recordDisposableObject)
    {
      ...
      if (this.disposed)
        throw new ObjectDisposedException("ContentManager");
      int length = Math.Max(assetName.LastIndexOf('\\'), assetName.LastIndexOf('/'));
      this.CurrentAssetDirectory = length == -1 ? this.RootDirectory : assetName.Substring(0, length);
      ...
      {
        assetName = TitleContainer.GetFilename(Path.Combine(this.RootDirectoryFullPath, assetName));
      ...
      if (obj == null)
        throw new ContentLoadException("Could not load " + originalAssetName + " asset!");
      this.CurrentAssetDirectory = (string) null;
      return (T) obj;
    }
    ...
    private ContentReader GetContentReaderFromXnb(string originalAssetName, ref Stream stream, BinaryReader xnbReader, Action<IDisposable> recordDisposableObject)
    {
      ...
      if (flag)
      {
        ...
        while (offset - position < (long) num7)
        {
          ...
          int inLen = num10 << 8 | num11;
          int outLen = 32768;
          long num12;
          if (num10 == (int) byte.MaxValue)
          {
            outLen = num11 << 8 | (int) (byte) stream.ReadByte();
    ...
    protected virtual void ReloadAsset<T>(string originalAssetName, T currentAsset)
    {
      ...
      {
        string assetName = this.Normalize<T>(TitleContainer.GetFilename(Path.Combine(this.RootDirectoryFullPath, str)));
