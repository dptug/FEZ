...
namespace Microsoft.Xna.Framework.Storage
{
  public class StorageContainer : IDisposable
  {
    ...
    internal StorageContainer(StorageDevice device, string name, PlayerIndex? playerIndex)
    {
      ...
      string storageRoot = StorageDevice.StorageRoot;
      this._storagePath = Path.Combine(Path.Combine(storageRoot, "SavedGames"), name);
      string str = string.Empty;
      if (playerIndex.HasValue)
        str = Path.Combine(storageRoot, "Player" + (object) playerIndex.Value);
      if (!string.IsNullOrEmpty(str))
        this._storagePath = Path.Combine(storageRoot, "Player" + (object) playerIndex.Value);
