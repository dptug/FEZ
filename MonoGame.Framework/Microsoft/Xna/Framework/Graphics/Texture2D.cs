...
namespace Microsoft.Xna.Framework.Graphics
{
  public class Texture2D : Texture
  {
    ...
    protected OpenTK.Graphics.OpenGL.PixelFormat glFormat;
    ...
    {
      byte[] data = new byte[width * height * GraphicsExtensions.Size(this.Format)];
      this.GetData<byte>(data);
      using (System.Drawing.Bitmap bitmap = new System.Drawing.Bitmap(width, height, System.Drawing.Imaging.PixelFormat.Format24bppRgb))
      {
        ...
        {
          Marshal.WriteByte(bitmapdata.Scan0, index * 3, data[index * 4 + 2]);
          Marshal.WriteByte(bitmapdata.Scan0, index * 3 + 1, data[index * 4 + 1]);
          Marshal.WriteByte(bitmapdata.Scan0, index * 3 + 2, data[index * 4]);
        }
        bitmap.UnlockBits(bitmapdata);
    ...
    internal void GenerateMipmaps()
    {
      Threading.BlockOnUIThread((Action) (() =>
      {
        ...
        this.levelCount = Texture.CalculateMipLevels(this.width, this.height, 0);
        GL.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, 9987);
        GL.BindTexture(TextureTarget.Texture2D, local_0);
      }));
    }
  }
